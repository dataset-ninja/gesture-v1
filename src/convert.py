...
def convert_and_upload_supervisely_project(
    ...
    images_path = os.path.join("archive","gesture_v1_yolov5","gesture_v1_yolov5","images")
    bboxes_path = os.path.join("archive","gesture_v1_yolov5","gesture_v1_yolov5","labels")
    bbox_ext = ".txt"
    batch_size = 30
    # gesture_v1_annotation == gesture_v1_yolov5
    ...
    def create_ann(image_path):
        ...
        bbox_path = os.path.join(curr_bboxes_path, get_file_name(image_path) + bbox_ext)
    ...
    one = sly.ObjClass("one", sly.Rectangle)
    two = sly.ObjClass("two", sly.Rectangle)
    three = sly.ObjClass("three", sly.Rectangle)
    four = sly.ObjClass("four", sly.Rectangle)
    five = sly.ObjClass("five", sly.Rectangle)

    idx_to_class = {0: one, 1: two, 2: three, 3: four, 4: five}
    ...
    for ds_name in os.listdir(images_path):
        ...
        all_images = os.listdir(curr_images_path)
        ...
        for img_names_batch in sly.batched(all_images, batch_size=batch_size):
            img_pathes_batch = [os.path.join(curr_images_path, im_name) for im_name in img_names_batch]
